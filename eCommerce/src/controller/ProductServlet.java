...
@WebServlet(name = "/ProductServlet", urlPatterns = { "/deleteProduct", "/addProduct", "/editProduct",
		"/addProduct.jsp", "/editProduct.jsp", "/deleteProduct.jsp"})
public class ProductServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (userPath.equals("/deleteProduct") || userPath.equals("/deleteProduct.jsp")) {
			...
			if (user != null) {
				if (user.getUsername().contentEquals("admin")) {
					...
					List<OrderedProduct> orderedProducts = orderedProductSB.findAll();
					int check = 0;
					for (OrderedProduct op : orderedProducts) {
						if (op.getProduct().equals(selectedProduct)) {
							check = 1;
							break;
						}
					}
					if (selectedProductDetail.getQuantity() == 0 && check == 0) {
			...
			//			categorySB.remove(categorySB.find(selectedProduct.getProductId()));
						...
						session.setAttribute("errDelProduct", "errDelProduct");
						userPath = "product";
					}
				} else {
					userPath = "requestNA";
		...
		else if (userPath.equals("/addProduct") || userPath.equals("/addProduct.jsp")) {
			userPath = "requestNA";
		} else if (userPath.equals("/editProduct") || userPath.equals("/editProduct.jsp")) {
			userPath = "requestNA";
		}
